...
`run.bat` は引数をアプリに渡す（`run.bat --no-post` / `run.bat --console` 等）。**起動オプションは今後体系化して整理する予定（--run-once / --topic / --config 等を検討中）**。
...
## 11. 構想（未実装・別ブランチ/ワークスペースで検討するもの）

### 11.1 Wiki / 参考文献 URL 欄（2026-08-04 ユーザー提案）

**動機**: トピックに詳しくない話題だと、AI が「すごく変なこと」を書く。参考文献（Wiki 等）への URL をトピック設定に持てると正確になる。

**トークンコストの懸念（ユーザー指摘）**: OMP は毎回新しいセッションで実行するため、Wiki の内容を毎回読み込むとコンテキスト・トークンの無駄。

**軽量案（推奨）**: `TopicConfig.reference_urls: Vec<String>` を追加し、**URL リストだけをプロンプトに埋め込む**（1 URL ≈ 40-80 トークン、5 URL でも数百トークン = 記事 40 件プロンプトの 2-3%）。OMP エージェントは Web ツールで**必要なときだけ**ページを読むので、毎回の読み込みコストは発生しない。

**重い案（別ブランチ）**: トピック単位で OMP セッションを再利用（resume）し、過去の Wiki 調査結果を引き継ぐ。履歴が肥大化するリスク（トークンコストがセッションの長さに比例）があるため、慎重に検証が必要。

### 11.2 トピック単位の OMP セッション再利用（2026-08-04 ユーザー提案）

- 同じトピックの実行はセッションを resume する（OMP のセッション機構）。
- 毎回新規セッションだと、トピックの前提知識（Wiki 調査結果など）を毎回捨てる。
- リスク: セッション履歴の肥大化・トークンコスト増・古い知識の固定化。**別ブランチで検証**。

### 11.3 起動オプションの体系化（2026-08-04 ユーザー指示）

現在 `--no-post` / `--console` の 2 つを実装済み。**体系化して整理する**（候補）:

| 候補 | 効果 |
|---|---|
| `--run-once` | 起動時にパイプラインを 1 回実行して終了（スケジューラ起動なし。CI/テスト用） |
| `--topic <name>` | 指定トピックだけ実行（--run-once と組み合わせ） |
| `--config <path>` | 設定ファイルパスを指定（テスト用） |
| `--verbose` / `--debug` | ログレベル変更（RUST_LOG の代替） |
| `--minimize` | 最小化/トレイのみで起動 |

実装時に `args` のパースを共通化（ヘルパー関数 or clap 導入）すること。
