...
### 11.4 構想メモ（2026-08-04 ユーザー提案・未実装）

1. **Reddit RSS のブラウザ UA による取得（429 回避の検証候補）**
   - ユーザー観察: ブラウザで Reddit の RSS をダウンロードすると通る（毎分制限に引っかからないっぽい）。
   - 仮説: Reddit はボット風 UA（MyQuickFeed/0.1）をレート制限し、ブラウザ風 UA は緩い。
   - 実測メモ: 2026-08-04 の調査では「デフォルト UA: 200 → 続けて 429」「説明的 UA: 429」だった（UA より IP ベースのレート制限の可能性も）。
   - 実装候補: `fetch_feed` で Reddit URL の場合のみブラウザ風 UA（例: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) ... Chrome/...`）を使う。**要検証**（UA を変えて 429 が減るか実測してから）。

2. **トピック詳細画面（Topics タブでトピックをクリック）**
   - 表示内容: ①そのトピックの設定情報（ソース・間隔・言語・reference_urls・フォーラムチャンネル等）②今までの OMP のプロンプトと回答（セッション履歴 = `%TEMP%\my-quick-feed\omp\omp-sessions\` の jsonl をパース）③実際に送信されたニュース（DB の posts テーブル）をまとめて見やすく表示。
   - 実装候補: TopicsPage にトピック詳細モーダル or 別ビュー。バックエンドに「トピックのセッション履歴を返す IPC」を追加（jsonl の user/assistant メッセージを抽出）。
   - セッション再利用（resume）が off の現在、セッションは実行ごとに新規作成されるため、履歴は複数ファイルに分散する点に注意（ファイル一覧からトピック名のプロンプトを含むものを特定する等の工夫が必要）。
