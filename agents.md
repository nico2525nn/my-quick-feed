...
**実装（軽量案・採用）**: `TopicConfig.reference_urls: Vec<String>`（serde default）を追加。プロンプトの「## 元記事」直前に「## 参考文献」セクションとして URL リストのみ埋め込む（空ならプロンプトは従来とバイト同一）。TopicsPage の編集モーダルに「Reference URLs（1行に1つ）」テキストエリア。**トークンコスト: URL 1 つ ≈ 40-80 トークンでほぼ無視できる**。OMP エージェントが Web ツールで**必要なときだけ**ページを読むので、毎回の読み込みコストは発生しない。
